...
print(len(usersInfo))
areaDict = {} #保存根据省市分区的信息
for userInfo in usersInfo:#userInfo为一个数组,第一个个位用户名,第二个为详细地址
    provinceindex = -1
    if '省' in  userInfo[1]:
        provinceindex = userInfo[1].find('省')
    elif '市' in  userInfo[1]:
        provinceindex = userInfo[1].find('市')
    else:
        continue
    ...
    province = userInfo[1][0:provinceindex+1]

    if province in areaDict:
        areaDict[province].append([userInfo[0],userInfo[1]])
    else:
        areaDict[province] = [[userInfo[0],userInfo[1]]]


for province in areaDict:
    print('province:',province)
    for info in areaDict[province]:
        print('    ',info)
...
for userInfo in usersInfo:#userInfo为一个数组,第一个个位用户名,第二个为详细地址
    ...
    if cityvalue in userInfo[1]:
        cityindex = userInfo[1].find(cityvalue)
    else:
        continue

    if provinceindex < 0 or cityindex < 1:
        continue

    province = userInfo[1][0:provinceindex+1]
    city = userInfo[1][0:cityindex+1]

    if province in areaDict:
        if city in areaDict[province]:
            areaDict[province][city].append([userInfo[0],userInfo[1]])
        else:
            areaDict[province][city] = [[userInfo[0],userInfo[1]]]
    else:
        areaDict[province] = {city:[[userInfo[0],userInfo[1]]]}
...
for province in areaDict:
    print('province:',province)
    for city in areaDict[province]:
        print('    city:',city)
        for info in areaDict[province][city]:
            print('        ',info)
